...
const snsParser = require('../../dist/decorators/snsMessageParser')
describe('decorators', function () {
  describe('snsMessageParser', function () {
    it('should return message content without the envelope', function () {
      ...
      expect(snsParser(request)).to.deep.equal({
        data: 'I am a test message'
      })
    })
    ...
    it('should throw an error if the message is not valid JSON', function () {
      ...
      expect(() => snsParser(request)).to.throw('The message body was not a valid JSON - maybe it is not comming from SNS')
    })
    ...
    it('should throw an error if the massage does note have a field .Message', function () {
      ...
      expect(() => snsParser(request)).to.throw('The message did not contain expected field (Message)')
    })
  })
})
